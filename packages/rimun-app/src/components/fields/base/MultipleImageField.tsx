...
export default function MultipleImageField({ name, max = 25 }: MultipleImageFieldProps) {
  const [{ value }, { error, touched }, { setValue }] = useField<ImageUpload[]>(name);
  ...
  const handleRemove = (idx: number) => {
    ...
    if (idx === 0) {
      ...
      const newValue = [...value.slice(0, idx), ...value.slice(idx + 1, value.length)];
      setValue(newValue);
    }
  };
          ...
          value.map((img, idx) => (
            <div key={idx} className="flex items-center py-2 justify-between">
              <div className="flex items-center">
                <Icon name="photograph" className="opacity-50" />
                <div className="ml-2 line-clamp-1 overflow-ellipsis">{img.name}</div>
              </div>
              <CircularButton icon="x" type="button" className="mr-2" onClick={() => handleRemove(idx)} />
            </div>
          ))}
      </div>
      ...
      {value.length < max && (
        <FieldItem {...{ error, touched }}>
          ...
          <CancelButton
            ...
            onClick={() => {
              const input = document.getElementById(`${name}-multiple-file-input`);
